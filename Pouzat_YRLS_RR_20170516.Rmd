...
```{r download-calibration-with-download.file,eval=FALSE}
download.file(url="http://xtof.disque.math.cnrs.fr/data/CCD_calibration.hdf5",
              destfile="CCD_calibration.hdf5")
```

Either way, once this is done we can check with `list.files` that the file is indeed on our hard-drive:
...
How can we test $\mathrm{ADU} \sim \mathrm{G}^2 \, \mathrm{F} + \sqrt{\mathrm{G}^2 \, (\mathrm{F}+\sigma_{ro}^2)} \, \epsilon$ ? Or how to set different values for $\mathrm{F}$?

Let's consider a pixel of our CCD "looking" at a fixed volume fluorescent material. We have two ways of modifying $\mathrm{F}$:
...
We introduce a rv $\mathrm{ADU}_{ij}$ for each pixel because it is very difficult (impossible) to have a uniform intensity ($i_e$) and a uniform volume ($v$) and a uniform quantum yield ($\phi$). We have therefore for each pixel:
